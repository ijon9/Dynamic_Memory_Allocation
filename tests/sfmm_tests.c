...
//############################################
Test(sfmm_basecode_suite, memalign_with_offset, .timeout = TEST_TIMEOUT) {
	size_t alignment = 256;
	sf_malloc(40);
	void *x = sf_memalign(50, alignment);
	cr_assert(!(((int)(long int)x)%alignment), "Memory is not aligned with %d.", alignment);
}

Test(sfmm_basecode_suite, memalign_without_offset, .timeout = TEST_TIMEOUT) {
	size_t alignment = 512;
	sf_malloc(456);
	void *x = sf_memalign(100, alignment);
	cr_assert(!(((int)(long int)x)%alignment), "Memory is not aligned with %d.", alignment);
}

Test(sfmm_basecode_suite, memalign_with_invalid_align, .timeout = TEST_TIMEOUT) {
	size_t alignment = 48;
	sf_malloc(456);
	void *x = sf_memalign(100, alignment);
...
Test(sfmm_basecode_suite, realloc_with_size_0, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(8);
	sf_malloc(8);
	// realloccing with size 0 should free the block
	x = sf_realloc(x, 0);
	assert_free_block_count(0, 2);
}

Test(sfmm_basecode_suite, malloc_one_page, .timeout = TEST_TIMEOUT) {
	sf_malloc(8136);
	assert_free_block_count(8144, 0);
}